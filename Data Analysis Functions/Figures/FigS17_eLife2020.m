...
%% Fig. S17
summaryFigure = figure('Name','FigS17 (a-d)');
sgtitle('Figure S17 - Turner et al. 2020')
%% [S17a] Peak cross-corr MUA
ax1 = subplot(2,2,1);
...
e3.MarkerSize = 10;
e3.CapSize = 10;
title({'[S17a] Peak cross-correlation MUA vs. \Delta[HbT] (\muM)','during arousal-states'})
...
%% [S17c] Peak cross-corr gamma-band
ax3 = subplot(2,2,3);
...
e3.MarkerSize = 10;
e3.CapSize = 10;
title({'[S17c] Peak cross-correlation gamma-band vs. \Delta[HbT] (\muM)','during arousal-states'})
ylabel('Peak corr. coef.')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
...
if strcmp(saveFigs,'y') == true
    ...
    set(summaryFigure,'PaperPositionMode','auto');
    print('-painters','-dpdf','-bestfit',[dirpath 'FigS17'])
    %% Text diary
    ...
    disp('======================================================================================================================')
    disp('[S17a] Generalized linear mixed-effects model statistics for peak MUA cross-corr during Rest, NREM, and REM')
    disp('======================================================================================================================')
    disp(muaPeakStats)
    disp('----------------------------------------------------------------------------------------------------------------------')
    disp(['Rest peak [MUA] (corrCoef): ' num2str(round(data.Rest.meanMuaPeak,1)) ' +/- ' num2str(round(data.Rest.stdMuaPeak,1))]); disp(' ')
    disp(['NREM peak [MUA] (corrCoef): ' num2str(round(data.NREM.meanMuaPeak,1)) ' +/- ' num2str(round(data.NREM.stdMuaPeak,1))]); disp(' ')
    disp(['REM peak [MUA] (corrCoef): ' num2str(round(data.REM.meanMuaPeak,1)) ' +/- ' num2str(round(data.REM.stdMuaPeak,1))]); disp(' ')
    disp('----------------------------------------------------------------------------------------------------------------------')
    % peak HbT statistical diary
    disp('======================================================================================================================')
    disp('[S17b] Generalized linear mixed-effects model statistics for MUA time-to-peak during Rest, NREM, and REM')
    disp('======================================================================================================================')
    disp(muaTTPStats)
    disp('----------------------------------------------------------------------------------------------------------------------')
    disp(['Rest TTP [MUA] (s): ' num2str(round(data.Rest.meanMuaTTP,1)) ' +/- ' num2str(round(data.Rest.stdMuaTTP,1))]); disp(' ')
    disp(['NREM  TTP [MUA] (s): ' num2str(round(data.NREM.meanMuaTTP,1)) ' +/- ' num2str(round(data.NREM.stdMuaTTP,1))]); disp(' ')
    disp(['REM  TTP [MUA] (s): ' num2str(round(data.REM.meanMuaTTP,1)) ' +/- ' num2str(round(data.REM.stdMuaTTP,1))]); disp(' ')
    disp('----------------------------------------------------------------------------------------------------------------------')
    % peak-to-peak D/D statistical diary
    disp('======================================================================================================================')
    disp('[S17c] Generalized linear mixed-effects model statistics for peak gamma-band cross-corr during Rest, NREM, and REM')
    disp('======================================================================================================================')
    disp(gammaPeakStats)
    disp('----------------------------------------------------------------------------------------------------------------------')
    disp(['Rest peak [gamma] (corrCoef): ' num2str(round(data.Rest.meanGammaPeak,1)) ' +/- ' num2str(round(data.Rest.stdGammaPeak,1))]); disp(' ')
    disp(['NREM peak [gamma] (corrCoef): ' num2str(round(data.NREM.meanGammaPeak,1)) ' +/- ' num2str(round(data.NREM.stdGammaPeak,1))]); disp(' ')
    disp(['REM peak [gamma] (corrCoef): ' num2str(round(data.REM.meanGammaPeak,1)) ' +/- ' num2str(round(data.REM.stdGammaPeak,1))]); disp(' ')
    disp('----------------------------------------------------------------------------------------------------------------------')
    % peak D/D statistical diary
    disp('======================================================================================================================')
    disp('[S17d] Generalized linear mixed-effects model statistics for gamma-band time-to-peak during Rest, NREM, and REM')
    disp('======================================================================================================================')
    disp(gammaTTPStats)
    disp('----------------------------------------------------------------------------------------------------------------------')
    disp(['Rest TTP [gamma] (s): ' num2str(round(data.Rest.meanGammaTTP,1)) ' +/- ' num2str(round(data.Rest.stdGammaTTP,1))]); disp(' ')
    disp(['NREM  TTP [gamma] (s): ' num2str(round(data.NREM.meanGammaTTP,1)) ' +/- ' num2str(round(data.NREM.stdGammaTTP,1))]); disp(' ')
    disp(['REM  TTP [gamma] (s): ' num2str(round(data.REM.meanGammaTTP,1)) ' +/- ' num2str(round(data.REM.stdGammaTTP,1))]); disp(' ')
    disp('----------------------------------------------------------------------------------------------------------------------')
    diary off
end
