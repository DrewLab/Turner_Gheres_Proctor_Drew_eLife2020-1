function [figHandle] = GenerateSingleFigures_IOS_eLife2020(procDataFileID,RestingBaselines,baselineType,saveFigs,imagingType,hemoType)
...
load(procDataFileID)
[animalID,fileDate,fileID] = GetFileInfo_IOS_eLife2020(procDataFileID);
...
p1 = plot((1:length(filtForceSensor))/ProcData.notes.dsFs,filtForceSensor,'color',colors_eLife2020('sapphire'),'LineWidth',1);
title([animalID ' IOS behavioral characterization and CBV dynamics for ' fileID2])
ylabel('Force Sensor (Volts)')
xlim([0,ProcData.notes.trialDuration_sec])
yyaxis right
p2 = plot((1:length(EMG))/ProcData.notes.dsFs,EMG,'color',colors_eLife2020('deep carrot orange'),'LineWidth',1);
ylabel('EMG (Volts^2)')
xlim([0,ProcData.notes.trialDuration_sec])
...
ax2 = subplot(6,1,2);
p3 = plot((1:length(filteredWhiskerAngle))/ProcData.notes.dsFs,-filteredWhiskerAngle,'color',colors_eLife2020('blue-green'),'LineWidth',1);
ylabel('Angle (deg)')
xlim([0,ProcData.notes.trialDuration_sec])
ylim([-20,60])
yyaxis right
p4 = plot((1:length(heartRate)),heartRate,'color',colors_eLife2020('dark sea green'),'LineWidth',1);
...
ax3 = subplot(6,1,3);
s1 = scatter((1:length(binForce))/ProcData.notes.dsFs,forceInds,'.','MarkerEdgeColor',colors_eLife2020('sapphire'));
hold on
s2 = scatter((1:length(binWhiskers))/ProcData.notes.dsFs,whiskInds,'.','MarkerEdgeColor',colors_eLife2020('blue-green'));
...
if strcmp(imagingType,'bilateral') == true
    if strcmp(hemoType,'reflectance') == true
        p5 = plot((1:length(filtLH_CBV))/ProcData.notes.CBVCamSamplingRate,filtLH_CBV,'color',colors_eLife2020('dark candy apple red'),'LineWidth',1);
        p6 = plot((1:length(filtRH_CBV))/ProcData.notes.CBVCamSamplingRate,filtRH_CBV,'color',colors_eLife2020('rich black'),'LineWidth',1);
        ylabel('\DeltaR/R (%)')
        legend([p5,p6,s1,s2,s3,s4,s5],'LH','RH','movement','whisking',',LPad sol','RPad sol','Aud sol')
    elseif strcmp(hemoType,'HbT') == true
        p5 = plot((1:length(filtLH_HbT))/ProcData.notes.CBVCamSamplingRate,filtLH_HbT,'color',colors_eLife2020('dark candy apple red'),'LineWidth',1);
        p6 = plot((1:length(filtRH_HbT))/ProcData.notes.CBVCamSamplingRate,filtRH_HbT,'color',colors_eLife2020('rich black'),'LineWidth',1);
        ylabel('\DeltaHbT')
        legend([p5,p6,s1,s2,s3,s4,s5],'LH','RH','movement','whisking',',LPad sol','RPad sol','Aud sol')
    end
elseif strcmp(imagingType,'single') == true
