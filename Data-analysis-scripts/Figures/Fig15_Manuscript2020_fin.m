function [] = Fig15_Manuscript2020_fin(rootFolder,AnalysisResults)
...
for aa = 1:length(IOSanimalIDs)
    ...
    for bb = 1:length(transitions)
        ...
        uniqueFileDates = AnalysisResults.(animalID).Transition.(transitions).fileDates;
        for cc = 1:length(uniqueFileDates)
            data.(animalID).(transition).(uniqueFileDates{cc,1}) = [];
        end
        for dd = 1:length(AnalysisResults.(animalID).Transition.(transitions).indFileDates)
            strDay = AnalysisResults.(animalID).Transition.(transitions).indFileDates{dd,1};
            data.(animalID).(transition).(strDay) = cat(1,data.(animalID).(transition).(strDay),AnalysisResults.(animalID).Transitions.(transition).HbT(dd,:));
        end
    end
end
% take average for each behavioral transition per day
for ee = 1:length(IOSanimalIDs)
    ...
    for ff = 1:length(transitions)
        ...
        for gg = 1:length(UnqiueFileDates)
            data.(transition).HbT{gg,1}(aa,:) = mean(data.(animalID).(transition).(uniqueFileDates{1,gg}),2);
        end
    end
end
%% Figure panel 4a
