    ...
    for zzz = 1:length(hemDataTypes)
        ...
        for aa = 1:length(dataTypes)
            ...
            if isempty(HbT_AllUnstimData) == false
                ...
                % save data and figures
                AnalysisResults.(animalID).NeuralHemoCoherence.AllUnstim.(dataType).(hemDataType).C = C_AllUnstimData;
                AnalysisResults.(animalID).NeuralHemoCoherence.AllUnstim.(dataType).(hemDataType).f = f_AllUnstimData;
                AnalysisResults.(animalID).NeuralHemoCoherence.AllUnstim.(dataType).(hemDataType).confC = confC_AllUnstimData;
                AnalysisResults.(animalID).NeuralHemoCoherence.AllUnstim.(dataType).(hemDataType).cErr = cErr_AllUnstimData;
                % save figures if desired
                if strcmp(saveFigs,'y') == true
            ...
            for dd = 1:length(modelTypes)
                ...
                % save data and figures
                AnalysisResults.(animalID).NeuralHemoCoherence.NREM.(modelType).(dataType).(hemDataType).C = C_nrem;
                AnalysisResults.(animalID).NeuralHemoCoherence.NREM.(modelType).(dataType).(hemDataType).f = f_nrem;
                AnalysisResults.(animalID).NeuralHemoCoherence.NREM.(modelType).(dataType).(hemDataType).confC = confC_nrem;
                AnalysisResults.(animalID).NeuralHemoCoherence.NREM.(modelType).(dataType).(hemDataType).cErr = cErr_nrem;
                % save figures if desired
                if strcmp(saveFigs,'y') == true
                ...
                % save data and figures
                AnalysisResults.(animalID).NeuralHemoCoherence.REM.(modelType).(dataType).(hemDataType).C = C_rem;
                AnalysisResults.(animalID).NeuralHemoCoherence.REM.(modelType).(dataType).(hemDataType).f = f_rem;
                AnalysisResults.(animalID).NeuralHemoCoherence.REM.(modelType).(dataType).(hemDataType).confC = confC_rem;
                AnalysisResults.(animalID).NeuralHemoCoherence.REM.(modelType).(dataType).(hemDataType).cErr = cErr_rem;
                % save figures if desired
                if strcmp(saveFigs,'y') == true
                    remCoherence = figure;
                    plot(f_rem,C_rem,'k')
