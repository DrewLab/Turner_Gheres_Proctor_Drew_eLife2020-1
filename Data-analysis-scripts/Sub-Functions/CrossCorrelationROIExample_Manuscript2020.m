function [] = CrossCorrelationROIExample_Manuscript2020(rootFolder,AnalysisResults)
...
%%
suppFigure = figure;
sgtitle('Max pixel-wise cross-correlation for establishing ROI location')
% gamma cross correlation image
ax1 = subplot(2,2,1);
...
colormap(ax1,parula)
c1 = colorbar;
ylabel(c1,{'Corr. coefficient';'gamma power vs. reflectance'})
caxis([.25,0.5])
axis image
set(gca,'Ticklength',[0,0])
set(gca,'box','off')
% MUA cross correlation image
ax2 = subplot(2,2,3);
imagesc(AnalysisResults.(animalID).CrossCorrExample.muaCorrImgAvg*-1)
title('MUA power vs. pixel reflectance')
xlabel('Image size (pixels)')
ylabel('Image size (pixels)')
colormap(ax2,parula)
c2 = colorbar;
ylabel(c2,{'Corr. coefficient';'MUA power vs. reflectance'})
caxis([0.1,0.35])
axis image
set(gca,'Ticklength',[0,0])
set(gca,'box','off')
% original image with circular ROI
ax3 = subplot(2,2,[2,4]);
...
title('final ROI placement')
xlabel('Image size (pixels)')
ylabel('Image size (pixels)')
colormap(ax3,gray)
c3 = colorbar;
ylabel(c3,{'Pixel Intensity'})
caxis([0,2^12])
axis image
set(gca,'Ticklength',[0,0])
set(gca,'box','off')
...
savefig(suppFigure, [dirpath 'Supplemental Figure - ROI Cross Correlation']);
