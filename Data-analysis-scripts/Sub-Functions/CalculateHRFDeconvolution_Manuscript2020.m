...
HRFDur = 5; % seconds
initvals = [1e-1,1,1,0,1e-2,0.1,0.05,0];
...
a_undr=((GamParams(6)/GamParams(7))^2*8*log10(2));
beta = ((GamParams(3)^2)/GamParams(2)/8/log10(2));
beta_undr=((GamParams(7)^2)/GamParams(6)/8/log10(2));
gamma = GamParams(1)*(t/GamParams(2)).^a.*exp((t - GamParams(2))/(-1*beta));
gamma_undr=GamParams(5)*(t/GamParams(6)).^a_undr.*exp((t-GamParams(6))/(-1*beta_undr));
double_gamma=gamma-gamma_undr;
% figure;
% p1 = plot(t,gamma);
% hold on
% p2 = plot(t,gamma_undr);
% p3 = plot(t,double_gamma);
% legend([p1,p2,p3],'gamma','gamma_undr','double_gamma')
% save results
AnalysisResults.(animalID).HRFs.(neuralBand).(hemisphere).(behavior).gammaFunc = double_gamma;
