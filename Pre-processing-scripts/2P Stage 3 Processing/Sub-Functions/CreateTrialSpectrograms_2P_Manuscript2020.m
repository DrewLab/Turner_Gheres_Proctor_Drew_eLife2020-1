function CreateTrialSpectrograms_2P_Manuscript2020(mergedDataFileIDs,neuralDataTypes)
...
for a = 1:size(mergedDataFileIDs, 1)
    ...
    if ~exist(specDataFileID,'file') == true
        ...
        for z = 1:length(neuralDataTypes)
            neuralDataType = neuralDataTypes{1,z};
            try
                rawNeuro = detrend(MergedData.data.(neuralDataType)(1:expectedLength),'constant');
            catch
                sampleDiff = expectedLength - length(MergedData.data.(neuralDataType));
                rawNeuro = detrend(horzcat(MergedData.data.(neuralDataType),MergedData.data.(neuralDataType)(end)*ones(1,sampleDiff)),'constant');
            end
            w0 = 60/(anFs/2);
            bw = w0/35;
