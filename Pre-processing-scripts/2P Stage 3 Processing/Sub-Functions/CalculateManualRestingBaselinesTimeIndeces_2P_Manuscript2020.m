        ...
        for ff = 1:length(subDataTypes)
            ...
            for nn = 1:length(uniqueDays)
                ...
                for pp = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{nn,1} = ConvertDate_2P_Manuscript2020(uniqueDays{nn,1});
        ...
        for ss = 1:length(subDataTypes)
            ...
            for aaa = 1:length(uniqueDays)
                ...
                for bbb = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{aaa,1} = ConvertDate_2P_Manuscript2020(uniqueDays{aaa,1});
            ...
            for ddd = 1:length(uniqueDays)
                uniqueDate{ddd,1} = ConvertDate_2P_Manuscript2020(uniqueDays{ddd,1});
                for eee = 1:length(subFilterFileList_B)
                    [~,~,fileDate,~,~,~] = GetFileInfo2_2P_Manuscript2020(subFilterFileList_B{eee,1});
                ...
                for fff = 1:length(validFileList)
                    [~,~,~,vesselFileIDs{fff,1},~,vesselIDs{fff,1}] = GetFileInfo2_2P_Manuscript2020(validFileList{fff,1});
                end
            ...
            for ggg = 1:length(uniqueDays)
                uniqueDate{ggg,1} = ConvertDate_2P_Manuscript2020(uniqueDays{ggg,1});
            ...
            for nnn = 1:length(subFilterFileList_B)
                [~,~,~,~,~,vessels{nnn,1}] = GetFileInfo2_2P_Manuscript2020(subFilterFileList_B{nnn,1});
            end
            vIDs = unique(vessels);
            % find the means of each unique day
