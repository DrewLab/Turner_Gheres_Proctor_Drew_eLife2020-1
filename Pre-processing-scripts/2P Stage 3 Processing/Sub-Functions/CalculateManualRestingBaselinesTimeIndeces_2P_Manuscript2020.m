function [RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_2P_Manuscript2020
...
for a = 1:size(mergedDataFileIDs,1)
    disp(['Loading file ' num2str(a) ' of ' num2str(size(mergedDataFileIDs,1)) '...']); disp(' ')
    mergedDataFileID = mergedDataFileIDs(a,:);
    load(mergedDataFileID,'-mat')
    ManualDecisions.fileIDs{a,1} = mergedDataFileIDs(a,:);
    if isfield(MergedData,'manualBaselineInfo') == false
        ...
        while b == false
            ...
            [singleTrialFig] = GenerateSingleFigures_2P_Manuscript2020(mergedDataFileID,baselineType,saveFigs,RestingBaselines);
        ...
        ManualDecisions.validFiles{a,1} = MergedData.manualBaselineInfo.fileDecision;
        ManualDecisions.startTimes{a,1} = MergedData.manualBaselineInfo.startTime;
        ManualDecisions.endTimes{a,1} = MergedData.manualBaselineInfo.endTime;
    end
end
...
for d = 1:length(ManualDecisions.validFiles)
    if strcmp(ManualDecisions.validFiles{d,1},'y') == true
        ...
        subFilterFileList_A{c,1} = ManualDecisions.fileIDs{d,1}(fileBreaks(2) + 1:fileBreaks(6) - 1); %#ok<*AGROW>
        subFilterFileList_B{c,1} = ManualDecisions.fileIDs{d,1};
...
for e = 1:length(dataTypes)
    ...
    if strcmp(dataType,'vesselDiameter') == false
        ...
        for f = 1:length(subDataTypes)
            ...
            uniqueDays = GetUniqueDays_2P_Manuscript2020(RestData.(dataType).(subDataType).fileIDs);
            ...
            for g = 1:length(uniqueDays)
                ...
                for j = 1:numberOfFiles
                    ...
                    for k = 1:length(subFilterFileList_A)
                        subFilterFile = subFilterFileList_A{k,1};
            ...
            for n = 1:length(filtFileIDs)
                ...
                for o = 1:length(subFilterFileList_A)
                    sFile = subFilterFileList_A{o,1};
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_2P_Manuscript2020(uniqueDays{p,1});
        ...
        for f = 1:length(subDataTypes)
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_2P_Manuscript2020(uniqueDays{p,1});
                    if strcmp(uniqueFileID_short,uniqueDays{p,1}) == 1
                        tempData.(uniqueDate{p,1}).data{q,1} = finalEventRestData{r,1};
                        tempData.(uniqueDate{p,1}).fileIDs{q,1} = finalEventFileIDs{r,1};
                        q = q + 1;
                    end
                end
            end
            ...
            for p = 1:length(uniqueDays)
                uniqueDate{p,1} = ConvertDate_2P_Manuscript2020(uniqueDays{p,1});
                for q = 1:length(subFilterFileList_B)
                    [~,~,fileDate,~,~,~] = GetFileInfo2_2P_Manuscript2020(subFilterFileList_B{q,1});
                    if strcmp(fileDate,uniqueDays{p,1}) == true
                        fileListFilter(q,1) = 1;
                    else
                        fileListFilter(q,1) = 0;
                    end
                end
                fileListFilter = logical(fileListFilter);
                validFileList = subFilterFileList_B(fileListFilter,:);
                for x = 1:length(validFileList)
                    [~,~,~,vesselFileIDs{x,1},~,vesselIDs{x,1}] = GetFileInfo2_2P_Manuscript2020(validFileList{x,1});
                end
                tempData.(uniqueDate{p,1}).vesselIDs = vesselIDs;
                tempData.(uniqueDate{p,1}).vesselFileIDs = vesselFileIDs;
            end
            %
            for p = 1:length(uniqueDays)
                uniqueDate{p,1} = ConvertDate_2P_Manuscript2020(uniqueDays{p,1});
                for x = 1:length(tempData.(uniqueDate{p,1}).vesselIDs)
                    vID = tempData.(uniqueDate{p,1}).vesselIDs{x,1};
                    vFile = tempData.(uniqueDate{p,1}).vesselFileIDs{x,1};
                    if isfield(tempData.(uniqueDate{p,1}),vID) == false
                        z = 1;
                        for y = 1:length(tempData.(uniqueDate{p,1}).fileIDs)
                            if strcmp(vFile,tempData.(uniqueDate{p,1}).fileIDs{y,1}) == true
                                tempData.(uniqueDate{p,1}).(vID).data{z,1} = tempData.(uniqueDate{p,1}).data{y,1};
                                z = z + 1;
                            end
                        end
                    else
                        z = length(tempData.(uniqueDate{p,1}).(vID).data) + 1;
                        for y = 1:length(tempData.(uniqueDate{p,1}).fileIDs)
                            if strcmp(vFile,tempData.(uniqueDate{p,1}).fileIDs{y,1}) == true
                                tempData.(uniqueDate{p,1}).(vID).data{z,1} = tempData.(uniqueDate{p,1}).data{y,1};
                                z = z + 1;
            ...
            %
            for a = 1:length(subFilterFileList_B)
                [~,~,~,~,~,vessels{a,1}] = GetFileInfo2_2P_Manuscript2020(subFilterFileList_B{a,1});
            end
            vIDs = unique(vessels);
            % find the means of each unique day
            for s = 1:size(uniqueDate,1)
                fields = fieldnames(tempData.(uniqueDate{s,1}));
                for q = 1:length(fields)
                    field = fields{q,1};
                    if ~strcmp(field,'data') && ~strcmp(field,'fileIDs') && ~strcmp(field,'vesselIDs') && ~strcmp(field,'vesselFileIDs')
                        tempDataMeans.(field).(uniqueDate{s,1}) = cellfun(@(x)mean(x),tempData.(uniqueDate{s,1}).(field).data);
                    end
                end
            end
            % save the means into the Baseline struct under the current loop iteration with the associated dates
            for s = 1:length(vIDs)
                dates = fieldnames(tempDataMeans.(vIDs{s,1}));
                for q = 1:length(dates)
                    date = dates{q,1};
                    RestingBaselines.manualSelection.(dataType).(subDataType).(vIDs{s,1}).(date) = mean(tempDataMeans.(vIDs{s,1}).(date));
                end
            end
        end
    end
