...
binWhiskers = LinkBinaryEvents_2P(gt(modBinWhiskers,0), [linkThresh breakThresh]*whiskerSamplingRate);
...
[MergedData.flags.whisk] = GetWhiskingData_2P(MergedData, binWhiskers);

% Identify and separate resting data
[MergedData.flags.stim] = GetStimData_2P(MergedData);

% Identify and separate resting data
[MergedData.flags.rest] = GetRestData_2P(MergedData);

% Save MergedData structure
save(fileName, 'MergedData');

function [puffTimes] = GetPuffTimes_2P(MergedData)
...
function [Stim] = GetStimData_2P(MergedData)
...
for sN = 1:length(solNames)
    ...
    for spT = 1:length(solPuffTimes) 
        ...
        % or whisked
        whiskScorePre = sum(MergedData.data.binWhiskerAngle(wPreStart:wPuffInd))...
            /(preTime*whiskerSamplingRate);
        whiskScorePost = sum(MergedData.data.binWhiskerAngle(wPuffInd:wPostEnd))...
            /(postTime*whiskerSamplingRate);
        moveScorePre = sum(MergedData.data.binForceSensorL(mPreStart:mPuffInd))...
            /(preTime*forceSensorSamplingRate);
        moveScorePost = sum(MergedData.data.binForceSensorL(mPuffInd:mPostEnd))...
...
function [Whisk] = GetWhiskingData_2P(MergedData, binarizedWhiskers)
%% Setup
whiskerSamplingRate = MergedData.notes.dsFs;
forceSensorSamplingRate = MergedData.notes.dsFs;
...
function [Rest] = GetRestData_2P(MergedData)
% Setup
whiskerSamplingRate = MergedData.notes.dsFs;
forceSensorSamplingRate = MergedData.notes.dsFs;
...
binarizedWhiskers = LinkBinaryEvents_2P(gt(modBinarizedWhiskers, 0), [linkThresh breakThresh]*whiskerSamplingRate);
binarizedForceSensor = LinkBinaryEvents_2P(modBinarizedForceSensor, [linkThresh breakThresh]*forceSensorSamplingRate);
