function [MScanData] = ExtractTiffAnalogData_2P_Manuscript2020(MScanData, fileID)
...
% 
% figure; plot(MScanData.data.corticalNeural)
% figure; plot(MScanData.data.EMG)
% figure; plot(MScanData.data.hippocampalNeural)
% figure; plot(MScanData.data.forceSensor)

disp('Analyzing vessel projections from defined polygons...'); disp(' ');
[MScanData] = GetDiameterFromMovie_2P_Manuscript2020(MScanData,fileID);

try
    [MScanData] = FWHM_MovieProjection_2P_Manuscript2020(MScanData,[MScanData.notes.startframe,MScanData.notes.endframe]);
catch
...
try
    ...
    if strcmp(MScanData.notes.vesselType,'D') || strcmp(MScanData.notes.vesselType,'V')
        MScanData.data.vesselDiameter = RemoveMotion_2P_Manuscript2020(MScanData.data.tempVesselDiameter,MScanData.notes.vesselROI.modalFixedDiameter,2,0.3);
    else
        MScanData.data.vesselDiameter = RemoveMotion_2P_Manuscript2020(MScanData.data.tempVesselDiameter,MScanData.notes.vesselROI.modalFixedDiameter,2,0.5);
    end
    [diamPerc,S,f] = DiamPercPower_2P_Manuscript2020(MScanData.data.vesselDiameter,MScanData.notes.vesselROI.modalFixedDiameter,MScanData.notes.frameRate);
...
function [MScanData] = GetDiameterFromMovie_2P_Manuscript2020(MScanData,fileID)
...
for theFrame = MScanData.notes.startframe:MScanData.notes.endframe
    ...
    [MScanData.notes.pixelShift(:,theFrame),~] = DftRegistration_2P_Manuscript2020(fftFirstFrame,fftRawFrame,1);
...
function [MScanData] = FWHM_MovieProjection_2P_Manuscript2020(MScanData, theFrames)
for f = min(theFrames):max(theFrames)
    % Add in a 5 pixel median filter
    MScanData.data.rawVesselDiameter(f) = CalcFWHM_2P_Manuscript2020(medfilt1(MScanData.notes.vesselROI.projection(f,:),5));
end
