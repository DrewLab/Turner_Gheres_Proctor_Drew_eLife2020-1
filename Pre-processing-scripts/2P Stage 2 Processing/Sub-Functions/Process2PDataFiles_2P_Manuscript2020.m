function [] = Process2PDataFiles_2P_Manuscript2020(labviewDataFiles,mscanDataFiles)
...
for a = 1:size(mscanDataFiles,1)
    ...
    if MScanData.notes.checklist.processData == false
        ...
        strDay = ConvertDate_2P_Manuscript2020(date);
        downSampledFs = 30;
        ...
        for q = 1:length(neuralTypes)
            ...
            % MUA [300-3000 Hz]
            [MScanData.data.(neuralField).muaPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'MUA',neuralType);
            % Gamma [30-100 Hz]
            [MScanData.data.(neuralField).gammaBandPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'Gam',neuralType);
            % Beta [13-30 Hz]
            [MScanData.data.(neuralField).betaBandPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'Beta',neuralType);
            % Alpha [10-13 Hz]
            [MScanData.data.(neuralField).alphaBandPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'Alpha',neuralType);
            % Theta [4-10 Hz]
            [MScanData.data.(neuralField).thetaBandPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'Theta',neuralType);
            % Delta [1-4 Hz]
            [MScanData.data.(neuralField).deltaBandPower] = ProcessNeuro_2P_Manuscript2020(MScanData,expectedLength,'Delta',neuralType);
        end
        ...
        [ok] = CheckForThreshold_2P_Manuscript2020(['binarizedForceSensor_' strDay],animalID);
        if ok == 0
            [forceSensorThreshold] = CreateForceSensorThreshold_2P_Manuscript2020(MScanData.data.dsForceSensorM);
            Thresholds.(['binarizedForceSensor_' strDay]) = forceSensorThreshold;
            save([animalID '_Thresholds.mat'],'Thresholds');
        end
        MScanData.data.binForceSensorM = BinarizeForceSensor_2P_Manuscript2020(MScanData.data.dsForceSensorM,Thresholds.(['binarizedForceSensor_' strDay]));
...
for b = 1:size(labviewDataFiles,1)
    ...
    if LabVIEWData.notes.checklist.processData == false
        ...
        [patchedWhisk] = PatchWhiskerAngle_2P_Manuscript2020(LabVIEWData.data.whiskerAngle,LabVIEWData.notes.whiskerCamSamplingRate_Hz,LabVIEWData.notes.trialDuration_Seconds,LabVIEWData.notes.droppedWhiskerCamFrameIndex);
        ...
        [ok] = CheckForThreshold_2P_Manuscript2020(['binarizedWhiskersLower_' strDay],animalID);
        if ok == 0
            [whiskersThresh1, whiskersThresh2] = CreateWhiskThreshold_2P_Manuscript2020(resampledWhisk,downSampledFs);
        ...
        binWhisk = BinarizeWhiskers_2P_Manuscript2020(resampledWhisk,downSampledFs,Thresholds.(['binarizedWhiskersLower_' strDay]),Thresholds.(['binarizedWhiskersUpper_' strDay]));
        [linkedBinarizedWhiskers] = LinkBinaryEvents_2P_Manuscript2020(gt(binWhisk,0),[round(downSampledFs/3),0]);
        ...
        [ok] = CheckForThreshold_2P_Manuscript2020(['binarizedForceSensor_' strDay],animalID);   
        if ok == 0
            [forceSensorThreshold] = CreateForceSensorThreshold_2P_Manuscript2020(LabVIEWData.data.dsForceSensorL);
            Thresholds.(['binarizedForceSensor_' strDay]) = forceSensorThreshold;
            save([animalID '_Thresholds.mat'],'Thresholds');
        end 
        LabVIEWData.data.binForceSensorL = BinarizeForceSensor_2P_Manuscript2020(LabVIEWData.data.dsForceSensorL,Thresholds.(['binarizedForceSensor_' strDay]));
