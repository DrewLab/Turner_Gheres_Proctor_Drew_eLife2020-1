function Analyze2PDiameter_2P_Manuscript2020(mscanDataFiles)
...
for a = 1:size(mscanDataFiles,1)
    ...
    if MScanData.notes.checklist.analyzeDiam == false
        if strcmp(MScanData.notes.movieType,'MS') == true || strcmp(MScanData.notes.movieType,'MD') == true
            [MScanData] = ExtractTiffAnalogData_2P_Manuscript2020(MScanData,[MScanData.notes.date '_' MScanData.notes.imageID]);
        ...
        elseif strcmp(MScanData.notes.movieType,'MP') == true
            [MScanData] = CalcPenVesselArea_2P_Manuscript2020(MScanData,[MScanData.notes.date '_' MScanData.notes.imageID]);
            MScanData.notes.checklist.analyzeDiam = true;         
            save([MScanData.notes.animalID '_' MScanData.notes.date '_' MScanData.notes.imageID '_' MScanData.notes.vesselID '_MScanData'],'MScanData')        
        elseif strcmp(MScanData.notes.movieType,'C') == true
            [MScanData] = GetVelocityLineScan_2P_Manuscript2020(MScanData,[MScanData.notes.date '_' MScanData.notes.imageID]);
            MScanData.notes.checklist.analyzeDiam = true;        
            save([MScanData.notes.animalID '_' MScanData.notes.date '_' MScanData.notes.imageID '_' MScanData.notes.vesselID '_MScanData'],'MScanData')        
        end
    else
