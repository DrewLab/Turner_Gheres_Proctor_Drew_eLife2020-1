...
disp('Calculating the resting baselines for the entire duration of each unique day...'); disp(' ')
...
for a = 1:length(dataTypes)
    ...
    for b = 1:length(subDataTypes)
        ...
        % Save the means into the Baseline struct under the current loop iteration with the associated dates
        for h = 1:length(uniqueDays)
            RestingBaselines.entireDuration.(dataType).(subDataType).(date{h, 1}) = mean(tempData_means{h, 1});    % LH date-specific means
        end
        
    end
end

RestingBaselines.entireDuration.baselineFileInfo.fileIDs = finalFileIDs;
RestingBaselines.entireDuration.baselineFileInfo.eventTimes = finalFileEventTimes;
RestingBaselines.entireDuration.baselineFileInfo.durations = finalFileDurations;
