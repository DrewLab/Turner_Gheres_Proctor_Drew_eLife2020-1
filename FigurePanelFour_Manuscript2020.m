...
ylim([-10,100])
set(gca,'box','off')
% [B] Mean HbT distribution during different behaviors
subplot(2,3,2);
edges = -25:15:130;
...
title({'[B] Mean \DeltaHbT (\muM)','arousal-state distribution',''})
xlabel('\DeltaHbT (\muM)')
ylabel('Probability')
axis square
set(gca,'box','off')
axis tight
% [C] Mean heart rate during different behaviors
subplot(2,3,3)
HR_xInds = ones(1,length(IOSanimalIDs));
scatter(HR_xInds*1,data.Rest.HR,75,'MarkerEdgeColor','k','MarkerFaceColor',colorA,'jitter','on','jitterAmount',0.25);
hold on
e6 = errorbar(1,data.Rest.meanHR,data.Rest.stdHR,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e6.Color = 'black';
e6.MarkerSize = 10;
e6.CapSize = 10;
scatter(HR_xInds*2,data.Whisk.HR,75,'MarkerEdgeColor','k','MarkerFaceColor',colorD,'jitter','on','jitterAmount',0.25);
e7 = errorbar(2,data.Whisk.meanHR,data.Whisk.stdHR,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e7.Color = 'black';
e7.MarkerSize = 10;
e7.CapSize = 10;
scatter(HR_xInds*3,data.NREM.HR,75,'MarkerEdgeColor','k','MarkerFaceColor',colorB,'jitter','on','jitterAmount',0.25);
e8 = errorbar(3,data.NREM.meanHR,data.NREM.stdHR,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e8.Color = 'black';
e8.MarkerSize = 10;
e8.CapSize = 10;
scatter(HR_xInds*4,data.REM.HR,75,'MarkerEdgeColor','k','MarkerFaceColor',colorC,'jitter','on','jitterAmount',0.25);
e9 = errorbar(4,data.REM.meanHR,data.REM.stdHR,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e9.Color = 'black';
e9.MarkerSize = 10;
e9.CapSize = 10;
title({'[C] Mean heart rate','during arousal states',''})
...
ylim([5,11])
set(gca,'box','off')
% [D] Peak vessel diameter during different behaviors
subplot(2,3,4)
...
e4.MarkerSize = 10;
e4.CapSize = 10;
title({'[D] Peak \DeltaD/D (%)','during arousal-states',''})
ylabel('\DeltaD/D (%)')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
...
savefig(summaryFigure,[dirpath 'Figure Panel Four']);
print('-painters','-dpdf','-fillpage',[dirpath 'Figure Panel Four'])

%% heart rate statistical diary
diary([dirpath 'FigurePanelFour_Statistics.txt'])
diary on
...
disp('======================================================================================================================')
disp('[D] Generalized linear mixed-effects model statistics for mean peak vessel diameter during Rest, Whisking, NREM, and REM')
disp('======================================================================================================================')
disp(vesselStats)
disp('----------------------------------------------------------------------------------------------------------------------')
...
disp('======================================================================================================================')
disp('[F] Generalized linear mixed-effects model statistics for mean doppler flow during Rest, Whisking, NREM, and REM')
disp('======================================================================================================================')
disp(flowStats)
disp('----------------------------------------------------------------------------------------------------------------------')
